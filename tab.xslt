			     ...
			     <xsl:otherwise>
			      <xsl:text>carry titleLink</xsl:text>
                      ...
                      <xsl:if test="$fileelement/maproomregistry:tabterm/@rdf:resource = $hr"> <!-- MAKE SURE THE MAPPAGE IS IN THE CURRENT TABTERM GROUP (THIS IS EFFECTIVELY THE INNER LOOP FOR A GROUP)-->
                            ...
                            <xsl:value-of select="($fileelement/iriterms:title[@xml:lang=$language],$fileelement/iriterms:title[1])[1]"/>
                            </a></div>
                            ...
                            <xsl:value-of select="($fileelement/iriterms:description[@xml:lang=$language],$fileelement/iriterms:description[1])[1]" disable-output-escaping="no"/></div>
